...
export default function TechnologiesSection() {
  const allTech = [...TECHNOLOGIES, ...TECHNOLOGIES]; // Duplicate for seamless loop
        ...
        <div className="relative w-full overflow-hidden">
          <div className="flex animate-marquee hover:[animation-play-state:paused]">
            {allTech.map((tech, index) => (
              <div
                key={`${tech.name}-${index}`}
                className="group flex flex-col items-center justify-center gap-3 text-center flex-shrink-0 mx-8"
              >
                <div className="flex h-20 w-20 items-center justify-center rounded-2xl bg-card/50 backdrop-blur-sm border border-dashed border-border/50 p-4 transition-all duration-300 group-hover:border-primary group-hover:-translate-y-1 group-hover:shadow-lg">
                  <TechIcon
                    tech={tech.name}
                    className="h-full w-full text-muted-foreground transition-colors group-hover:text-primary"
                  />
                </div>
